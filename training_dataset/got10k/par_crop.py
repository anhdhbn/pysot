...
def crop_img(img_files, anns, meta, set_crop_base_path, seq_name, instanc_size=511):
    frame_crop_base_path = join(set_crop_base_path, seq_name)
    ...
    for idx, img_file in enumerate(img_files):
        ...
        cv2.imwrite(join(frame_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(0, idx)), z)
        cv2.imwrite(join(frame_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(0, idx)), x)

def main(instanc_size=511, num_threads=12, input_dir="got10k", output_dir = "./"):
...
if __name__ == '__main__':
    ...
    input_dir="got10k"
    output_dir = "./"
    main(int(sys.argv[1]), int(sys.argv[2]), input_dir, output_dir)
