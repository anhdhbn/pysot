...
def calc_output_size(instance_size):
    conv2d = int((instance_size-7+2*0)/2+1)
    max_pool_2d = (conv2d-3+2*1)/2+1
    layer2 = (max_pool_2d-3+2*0)/2+1
    return int(layer2)
...
class SiamAttnRPNTracker(SiamRPNTracker):
    def __init__(self, model):
        ...
        self.score_size = calc_output_size(cfg.TRACK.INSTANCE_SIZE)
    ...
    def track(self, img):
        ...
        score_size = calc_output_size(instance_size)
